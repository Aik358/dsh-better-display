...
`0.3.0-preview.1` 从 `v0.2.1` 建立，目标是保持阅读体验，同时把官方新增内容和控件接回来。当前验收目标是 Harness `0.1.5-rc.2`。此版本供本机试用，不代表所有重构阶段已经完成。
...
试用安装应保留 `v0.2.1` 回退点，只更换已有客户端插件。无需新增 Host 或改写会话数据；合并主线和公开发布等待用户试用确认。
