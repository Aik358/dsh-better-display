...
const AssistantNode = memo(function AssistantNode({ useChat, nodeKey, boundary, processOpen = false, pinned = false, folded = false, processOnly = false, partStart, motion, onRead, returnFocusTo, ...render }) {
    const node = useChat(snapshot => snapshot.nodes.get(nodeKey));
...
const TurnGroup = memo(function TurnGroup({ group, motion, autoFold, processOnly, pinnedKeys, selectedProcessKeys, isAwaitingModel = false, ...props }) {
    ...
    const renderStep = (step, folded) => {
        ...
        if (step.kind === 'reasoning' || step.kind === 'body')
            return _jsx(BlockBoundary, { children: _jsx(AssistantNode, { ...shared, boundary: boundary, nodeKey: step.nodeKey, partStart: step.start, pinned: pinnedKeys.includes(step.nodeKey), processOpen: processOpen, folded: folded, processOnly: processOnly, motion: motion, onRead: pinProcess, returnFocusTo: processButton }) });
        if (step.kind === 'tool')
...
export function Reader(props) {
    ...
    const isAwaitingModel = useMemo(() => {
        ...
        if (!running && pendingSubmissions && pendingSubmissions.length > 0)
            return true;
    ...
    return _jsx(StreamMotionContext.Provider, { value: streamMotion, children: _jsxs("div", { ref: root, className: css.root, "data-dsh-better-display": "0.1.1", "data-reader-wait-clock-version": "input-v1", "data-reader-wait-start": waitAnchor.time ?? undefined, "data-motion": motion ? 'on' : 'off', children: [_jsx(TimelineRail, { items: timelineItems, activeTurn: activeTurn, busyTurn: busyTurn, onNavigate: onNavigateTurn }), _jsxs("div", { className: css.column, "data-chat-flow": "", children: [_jsxs(StickyLane, { kind: "toolbar", className: css.toolbar, children: [_jsx("button", { type: "button", className: css.textButton, "aria-pressed": autoFold, onClick: () => props.actions.setAutoFold(!autoFold), title: "\u65B0\u601D\u8003\u4EA7\u751F\u65F6\uFF0C\u662F\u5426\u81EA\u52A8\u5C06\u6B64\u524D\u6B65\u9AA4\u6536\u62E2\u4E3A\u4E00\u884C\u6C47\u603B\u3002\u5173\u95ED\u540E\u5B8C\u6574\u4FDD\u7559\u539F\u59CB\u8FC7\u7A0B\u4E0E\u6D41\u5F0F\u8F93\u51FA\u3002", children: `自动折叠${autoFold ? '开' : '关'}` }), _jsx("button", { type: "button", className: css.textButton, "aria-pressed": processOnly, onClick: () => props.actions.setProcessOnly(!processOnly), title: "\u53EA\u6298\u53E0\u601D\u8003\u4E0E\u5DE5\u5177\u8C03\u7528\uFF0C\u4FDD\u7559\u6BCF\u6BB5\u5BF9\u5916\u8F93\u51FA\u7684\u6B63\u6587\u5B8C\u6574\u53EF\u8BFB\u3002", children: `只折叠过程${processOnly ? '开' : '关'}` })] }), hasMore && _jsx("button", { type: "button", className: css.historyButton, disabled: loadingOlder, onClick: async () => {
                            ...
                            }, children: loadingOlder ? '正在加载更早记录' : '加载更早记录' }), historyError && _jsx("div", { className: css.notice, children: "\u5386\u53F2\u8BB0\u5F55\u52A0\u8F7D\u5931\u8D25\uFF0C\u53EF\u518D\u6B21\u5C1D\u8BD5\uFF1B\u73B0\u6709\u5185\u5BB9\u672A\u6539\u53D8\u3002" }), openError && _jsxs("div", { className: css.error, role: "alert", children: ["\u4F1A\u8BDD\u6682\u65F6\u65E0\u6CD5\u8BFB\u53D6\uFF1A", openError.message] }), loading && groups.length === 0 && _jsx("p", { className: css.empty, role: "status", children: "\u6B63\u5728\u8BFB\u53D6\u4F1A\u8BDD\u2026" }), groups.map(group => _jsx(TurnGroup, { ...props, group: group, motion: motion, autoFold: autoFold, processOnly: processOnly, pinnedKeys: pinnedKeys, selectedProcessKeys: selectedProcessKeys, isAwaitingModel: isAwaitingModel && group.key === groups.at(-1)?.key }, group.key)), visibleSubmissions.map(submission => (_jsxs("div", { className: css.userCluster, "data-reader-pending-submission": true, children: [submission.attachments.some(item => item.type === 'image') && (_jsx("div", { className: css.userImages, children: submission.attachments.map((item, idx) => item.type === 'image' ? (_jsx("figure", { className: css.imageFigure, children: _jsx("div", { className: css.imageFrame, style: { aspectRatio: `${item.value.width || 4} / ${item.value.height || 3}` }, children: _jsx("img", { src: item.value.previewUrl, alt: item.value.name ?? '发送的图片', className: css.pendingImage }) }) }, idx)) : null) })), submission.text ? (_jsx("div", { className: css.user, children: _jsx("div", { className: css.blocks, children: submission.text }) })) : null, _jsx(UserMessageActions, { text: submission.text, time: submission.time })] }, submission.requestId))), isAwaitingModel && _jsx(WaitingStatus, { anchor: waitAnchor, label: props.t ? props.t('chat.deepDiving') : '深度求索中...' }), pending !== undefined && _jsxs("div", { className: css.attention, role: "alert", "data-reader-attention": true, children: [_jsx("strong", { children: pending.kind === 'question' ? '需要你回答一个问题' : '需要你的确认' }), _jsx("span", { children: "\u8BF7\u5728\u4E0B\u65B9\u539F\u751F\u64CD\u4F5C\u533A\u5904\u7406\u3002\u6B64\u63D0\u793A\u4E0D\u4F1A\u6536\u8FDB\u6267\u884C\u8FC7\u7A0B\u3002" })] }), scroll.detached && _jsx("div", { className: css.jumpDock, children: _jsx("button", { type: "button", className: css.jump, "aria-label": "\u56DE\u5230\u5E95\u90E8", title: "\u56DE\u5230\u5E95\u90E8", onClick: scroll.jump, children: _jsx("svg", { viewBox: "0 0 14 14", width: "14", height: "14", "aria-hidden": "true", children: _jsx("path", { d: "M3 5.5 7 9.5 11 5.5", fill: "none", stroke: "currentColor", strokeWidth: "1.4", strokeLinecap: "round", strokeLinejoin: "round" }) }) }) })] })] }) });
}
