...
import { foldIntensityOf, frostedGlassOf } from './fold-intensity.js';
...
const TurnGroup = memo(function TurnGroup({ group, motion, foldIntensity, pinnedKeys, selectedProcessKeys, isAwaitingModel = false, ...props }) {
...
export function Reader(props) {
    ...
    const prefsSnap = useSyncExternalStore(props.openPrefs?.subscribe ?? ((fn) => { void fn; return () => { }; }), () => props.openPrefs?.getSnapshot() ?? {}, () => ({}));
    ...
    return _jsx(StreamMotionContext.Provider, { value: streamMotion, children: _jsxs("div", { ref: root, className: css.root, "data-dsh-better-display": "0.1.1", "data-reader-wait-clock-version": "input-v1", "data-reader-wait-start": waitAnchor.time ?? undefined, "data-motion": motion ? 'on' : 'off', "data-reader-glass": frostedGlass || undefined, "data-reader-fold-intensity": foldIntensity, children: [_jsx(TimelineRail, { items: timelineItems, activeTurn: activeTurn, busyTurn: busyTurn, onNavigate: onNavigateTurn }), _jsxs("div", { className: css.column, "data-chat-flow": "", children: [_jsx(StickyLane, { kind: "toolbar", className: css.toolbar, children: _jsx("div", { className: css.foldControl, role: "radiogroup", "aria-label": "\u8FC7\u7A0B\u6298\u53E0\u5F3A\u5EA6", "data-reader-fold-control": foldIntensity, children: [
                                    [0, '不折叠', '全程展开思考、工具与正文，不做自动折叠。'],
                                    [1, '标准折叠', '新思考出现时，按当前主线收起此前步骤。'],
                                    [2, '过程摘要', '已结束的思考和工具收成一行摘要，正文永不折叠。'],
                                ].map(([value, label, title]) => (_jsx("button", { type: "button", role: "radio", "aria-checked": foldIntensity === value, className: css.foldStop, "data-on": foldIntensity === value || undefined, "data-reader-fold-stop": value, title: title, onClick: () => { props.openPrefs?.actions?.setFoldIntensity?.(value); }, children: label }, value))) }) }), hasMore && _jsx("button", { type: "button", className: css.historyButton, disabled: loadingOlder, onClick: async () => {
