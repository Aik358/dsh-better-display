...
function MorphPanel({ open, children, onSettled, onClosed }) {
    ...
    useLayoutEffect(() => {
        ...
        animation.onfinish = () => { release(); if (open)
            onSettled?.(element);
        else
            onClosed(); };
        return () => { animation.cancel(); release(); };
    }, [open]);
    return _jsx("span", { ref: ref, className: css.diffOverlay, children: children });
}
/**
...
export function DiffStat({ steps, label }) {
    ...
    return _jsxs("span", { className: css.diffStatRoot, children: [_jsxs("button", { type: "button", className: css.diffStatButton, "aria-expanded": open, "aria-label": `${label} · 改动 ${totals.added} 行，删除 ${totals.removed} 行`, onClick: event => {
                    ...
                    setVisible(true);
                    setOpen(true);
                }, children: [_jsx("span", { className: css.diffCaret, "aria-hidden": true, children: open ? '\u25be' : '\u25b8' }), totals.added > 0 && _jsxs("span", { className: css.diffAdded, children: ["+", totals.added] }), totals.removed > 0 && _jsxs("span", { className: css.diffRemoved, children: ["-", totals.removed] })] }), visible && _jsxs(MorphPanel, { open: open, onSettled: revealPanel, onClosed: () => setVisible(false), children: [hunks.length > 1 && _jsx("span", { className: css.diffTabs, children: hunks.map((hunk, index) => _jsx("button", { type: "button", className: css.diffTab, "data-active": index === active || undefined, onClick: () => setActive(index), title: hunk.path, children: hunk.path.split(/[/\\]+/).filter(Boolean).slice(-1)[0] ?? hunk.path }, `${hunk.path}:${index}`)) }), _jsx(DiffBlock, { diffs: [current], maxLines: 24, labels: diffBlockLabels })] })] });
}
