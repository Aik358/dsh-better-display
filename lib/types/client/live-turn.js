...
export function processSummary(steps) {
    let reasoning = 0;
    let extra = 0;
    const tools = [];
    for (const step of steps) {
        if (step.kind === 'reasoning')
            ...
            tools.push(toolSummary(step.entry));
    }
    const parts = [];
    if (reasoning)
        parts.push(reasoning > 1 ? '思考\u00d7' + reasoning : '思考');
    if (tools.length) {
        parts.push(tools.slice(0, 2).join('\u3001'));
        if (tools.length > 2)
            parts.push('\u7b49 ' + tools.length + ' \u6b65');
    }
    if (extra)
        parts.push('记录\u00d7' + extra);
 ...
 */
export function splitProcessChain(chain, turnOpen) {
    const items = [];
    let pending = [];
    const commit = () => {
        if (!pending.length)
    ...
    for (const step of chain) {
        ...
        if (step.kind === 'reasoning' && pending.length)
            commit();
        pending.push(step);
    }
    if (turnOpen) {
...
export function presentForIntensity(steps, boundary, intensity) {
    return presentLiveTurn(steps, boundary, autoFoldFromIntensity(intensity), processOnlyFromIntensity(intensity));
}
export function presentLiveTurn(steps, boundary, autoFold = true, processOnly = false) {
